...
function spaceinvaders() {
    ...
    const gameClock = rxjs_1.interval(10)
        .pipe(operators_1.map(elapsed => new Tick(elapsed))), keyObservable = (e, k, result) => rxjs_1.fromEvent(document, e)
        .pipe(operators_1.filter(({ code }) => code === k), operators_1.filter(({ repeat }) => !repeat), operators_1.map(result)), startLeft = keyObservable('keydown', 'ArrowLeft', () => new moveShip(-5)), startRight = keyObservable('keydown', 'ArrowRight', () => new moveShip(5)), stopLeft = keyObservable('keyup', 'ArrowLeft', () => new stopShip(0)), stopRight = keyObservable('keyup', 'ArrowRight', () => new stopShip(0)), shoot = keyObservable('keydown', 'Space', () => new Shoot()), moveEnemies = rxjs_1.interval(10).pipe(operators_1.map((_) => new moveAlien())), restartEnter = keyObservable('keydown', 'Enter', () => new restartGame()), shootEnemy = rxjs_1.interval(1000).pipe(operators_1.map((_) => new shootAlien()));
    // Aliens, shields and bullets are all circles
    // adapted from Asteroids code by Tim Dwyer:
    // https://tgdwyer.github.io/asteroids/
    const createCircle = (viewType) => (oid) => (circ) => (vel) => ({
        ...oid,
        ...circ,
        vel: vel,
        acc: Vec.Zero,
        ...
        id: viewType + oid.id,
        viewType: viewType
    }), createAlien = createCircle('alien'), createBullet = createCircle('bullet'), createEnemyBullet = createCircle('enemyBullet'), createShield = createCircle('shield');
    function createShip() {
        return {
            id: 'ship',
            viewType: 'ship',
    ...
    const spawnAliens = [...Array(Constants.StartAliensCount)]
        ...
        ? // id of created alien greater between 0 and 4 are placed on first row
            createAlien({ id: String(i), createTime: Constants.StartTime })({ pos: new Vec(i * 75 + 250, 30), radius: Constants.StartAlienRadius })(new Vec(2, 0))
        : // id of created alien greater between 5 and 9 are placed on second row
            createAlien({ id: String(i), createTime: Constants.StartTime })({ pos: new Vec(i * 75 - 90, 90), radius: Constants.StartAlienRadius })(new Vec(2, 0)));
    ...
    const spawnShields = [...Array(Constants.StartShieldCount)]
        ...
        createShield({ id: String(i), createTime: Constants.StartTime })({ pos: new Vec(i * 5 + 80, 350), radius: Constants.StartShieldRadius })(new Vec(0, 0)) // velocity, movement
        : (i >= 21 && i <= 40) ? // id of created shields between 21 and 40 are lumped toegther as middle shield
            createShield({ id: String(i), createTime: Constants.StartTime })({ pos: new Vec(i * 5 + 230, 350), radius: Constants.StartShieldRadius })(new Vec(0, 0)) // id of created shields between 41 and 60 are lumped toegther as rightmost shield
            : createShield({ id: String(i), createTime: Constants.StartTime })({ pos: new Vec(i * 5 + 380, 350), radius: Constants.StartShieldRadius })(new Vec(0, 0)));
    ...
    const torusWrap = ({ x, y }) => {
        ...
        // adapted from Asteroids code by Tim Dwyer:
        // https://tgdwyer.github.io/asteroids/
        const s = Constants.CanvasSize, wrap = (v) => v < 20 ? 20 : v > s - 20 ? 780 : v;
    ...
    const handleCollisions = (s) => {
        const cut = except((a) => (b) => a.id === b.id), bodiesCollided = ([a, b]) => a.pos.sub(b.pos).len() < a.radius + b.radius, allBodiesShieldBullet = (body1, body2) => flatMap(body1, b1 => body2.map(b2 => ([b1, b2]))), 
        // Ship collide with alien body
        shipCollided = s.aliens.filter(r => bodiesCollided([s.ship, r])).length > 0, allBulletsAndAliens = flatMap(s.bullets, b => s.aliens.map(r => ([b, r]))), 
        // Ship bullets collide with alien body
        collidedBulletsAndAliens = allBulletsAndAliens.filter(bodiesCollided), collidedBullets = collidedBulletsAndAliens.map(([bullet, _]) => bullet), collidedAliens = collidedBulletsAndAliens.map(([_, alien]) => alien), 
        ...
        collidedShieldsAndAlienBullets = allBodiesShieldBullet(s.enemyBullets, s.shields).filter(bodiesCollided), collidedAlienBulletsandShield = collidedShieldsAndAlienBullets.map(([alienBullet, _]) => alienBullet), collidedShieldsandAlienBullet = collidedShieldsAndAlienBullets.map(([_, shield]) => shield), 
        // Alien Body collide with shields
        collidedShieldsAndAlien = allBodiesShieldBullet(s.aliens, s.shields).filter(bodiesCollided), collidedShieldswithAlienBody = collidedShieldsAndAlien.map(([_, shield]) => shield);
        return {
        ...
        return {
            ...
            aliens: s.aliens.map((obj) => ({
                ...
                vel: (aliensOutofBounds.length > 0) // changes alien's movement by adjusting velocity
                    ...
                    : obj.vel,
                pos: (aliensOutofBounds.length > 0) ? obj.pos.add(new Vec(0, 15)) : obj.pos,
            }))
        };
    };
    const makeAlienShoot = (s) => {
        ...
        return {
            ...
            enemyBullets: s.aliens.length != 0
                ? s.enemyBullets.concat([((unitVec) => createEnemyBullet({ id: String(s.objCount), createTime: s.time })({ radius: Constants.BulletRadius,
    ...
    const tick = (s, elapsed) => {
        const expired = (b) => (elapsed - b.createTime) > 70, expiredBullets = s.bullets.filter(expired), expiredEnemyBullets = s.enemyBullets.filter(expired), activeBullets = s.bullets.filter(not(expired)), activeEnemyBullets = s.enemyBullets.filter(not(expired));
    ...
    const reduceState = (s, e) => {
        ...
        return e instanceof moveShip ? { ...s,
            ...
            e instanceof stopShip ? { ...s,
                ...
                e instanceof Shoot ? { ...s, bullets: s.bullets.concat([((unitVec) => createBullet({ id: String(s.objCount), createTime: s.time })({ radius: Constants.BulletRadius,
    ...
    const subscription = rxjs_1.merge(gameClock, startLeft, startRight, stopLeft, stopRight, shoot, moveEnemies, restartEnter, shootEnemy)
        .pipe(operators_1.scan(reduceState, initialState))
    ...
    function updateView(s) {
        // adapted from Asteroids code by Tim Dwyer:
        // https://tgdwyer.github.io/asteroids/
        const svg = document.getElementById("svgCanvas"), ship = document.getElementById("ship"), score = document.getElementById("score"), level = document.getElementById("level"), show = (id, condition) => ((e) => condition ? e.classList.remove('hidden')
            : e.classList.add('hidden'))(document.getElementById(id)), updateBodyView = (b) => {
...
function showKeys() {
    ...
    // https://tgdwyer.github.io/asteroids/
    function showKey(k) {
        const arrowKey = document.getElementById(k), o = (e) => rxjs_1.fromEvent(document, e).pipe(operators_1.filter(({ code }) => code === k));
